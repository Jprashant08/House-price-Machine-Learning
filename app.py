...
@app.route('/')
def home():
    bhks = sorted(model.df['bhk'].unique())

    return render_template('index.html', bhks=bhks)
...
def predict():
    ...
    predicted_price = model.predict_house_price(location, bhk, total_sqft_int, availability)
